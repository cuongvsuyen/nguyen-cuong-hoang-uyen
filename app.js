(() => {
  ...
  function applyWeddingConfig() {
    ...
    // Bữa cơm thân mật: ngày ngoài phong bì, thời gian, lịch và địa điểm
    // đều lấy từ một nguồn duy nhất là reception.
    const r = parseDateOnly(cfg.reception?.date);
    setText('[data-config="gate-date"]', r.y ? `${r.d} tháng ${r.m}, ${r.y}` : '');
  ...
  function buildTimeline() {
    const root = $('#timeline');
    if (!root || !Array.isArray(cfg.timeline)) return;
  ...
  function buildBankCards() {
    ...
    root.innerHTML = '';
    const cards = Array.isArray(cfg.bankCards) ? cfg.bankCards : [];

    if (!cards.length) {
      root.innerHTML = '<p class="bank-empty">Chưa cấu hình thông tin mừng cưới.</p>';
      return;
    }
    ...
    cards.forEach(card => {
      ...
      const qrUrl = new URL(card.qr, document.baseURI).href;
      el.innerHTML = `
        <h3>${safeText(card.role)} - ${safeText(card.name)}</h3>
        <img class="bank-qr-image" src="${qrUrl}" alt="QR ${safeText(card.role)}" />
      ...
      const qrImage = $('.bank-qr-image', el);
      qrImage.addEventListener('error', () => {
        console.error('Không tải được QR:', qrUrl);
        qrImage.style.display = 'none';
        const error = document.createElement('p');
        error.className = 'bank-qr-error';
        error.textContent = 'Không tải được ảnh QR';
        qrImage.after(error);
      });
      ...
      save.addEventListener('click', async event => {
        ...
        const filename = `qr-${String(card.role || 'wedding').toLowerCase().replace(/\s+/g, '-')}.png`;
        try {
          ...
          console.error('Không download được QR:', error);
  ...
  function submitRsvpToGoogle(endpoint, fields) {
    return new Promise((resolve, reject) => {
      ...
      const onMessage = event => {
        ...
        /*
         * Apps Script HtmlService có thể chạy response trong một iframe sandbox
         * lồng bên trong iframe target. Vì vậy KHÔNG kiểm tra event.source.
         * requestId là giá trị ngẫu nhiên riêng của từng submit và là điều kiện
         * dùng để xác định đúng ACK của request hiện tại.
         */
        if (
          !data ||
        ...
        if (data.ok) {
          finish(resolve, { ...data, confirmed: true });
        } else {
          finish(
            reject,
            new Error(data.error || 'Google Sheet không thể lưu RSVP')
          );
        }
      };
      ...
      iframe.addEventListener('load', () => {
        if (phase === 'boot') {
          // about:blank đã sẵn sàng -> mới submit form để lần load tiếp theo
          // chắc chắn là response từ Apps Script.
          phase = 'submitted';
        ...
        if (phase === 'submitted') {
          ...
          /*
           * Nếu Google đã trả response nhưng sandbox chặn postMessage ở một
           * browser nào đó, coi request là đã được gửi thành công thay vì báo
           * "chưa gửi" sai. ACK chính xác vẫn được ưu tiên trong 1.2 giây đầu.
           */
      ...
      const timeoutTimer = setTimeout(() => {
        finish(
          reject,
          new Error('Hết thời gian chờ phản hồi từ Google Apps Script')
        );
      }, 30000);
  ...
  $('#addCalendar').addEventListener('click', () => {
    ...
    const ics = [
      'BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Long Phung Family Meal//VI', 'BEGIN:VEVENT',
      `DTSTART:${format(start)}`, `DTEND:${format(end)}`,
      `SUMMARY:Bữa cơm thân mật - ${cfg.couple.groomShort} & ${cfg.couple.brideShort}`,
      `LOCATION:${cfg.reception.venue}`,
      'DESCRIPTION:Thân mời bạn tới dự bữa cơm thân mật cùng gia đình chúng tôi.',
